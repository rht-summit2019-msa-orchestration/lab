...
The several services are implemented using the runtimes provided by Red Hat OpenShift Application Runtimes. More specifically, Spring Boot for the Passenger service and the Dispatch service, Vert.x for the driver service. The Dispatch service uses the RHPAM _Spring Boot starter_ to embed the RHPAM engine in a Spring Boot application.
