...
Microservice architectures are still hot. The idea of breaking up large monolith applications into smaller, focused, independently deployable services is attractive to a lot of development groups. The increased adoption of cloud deployment platforms like OpenShift is an accelerator for the adoption of microservice architectures. The promise of microservice architectures is increased agility and reduced time to market.
...
The passenger service implementation used in the lab is a mocked service, which does hoever simulate the message exchanges between the passenger service and the other services..
...
The passenger service implementation used in the lab is a mocked service, which does simulate the message exchanges between the driver service and the other services.
...
This means that message consumers need to filter on the specific event types that they are interested in and discard others.

The different services are implemented using the runtimes provided by Red Hat OpenShift Application Runtimes. More specifically, Spring Boot for the Passenger service and the Dispatch service, Vert.x for the driver service. The Dispatch service uses the RHPAM _Spring Boot starter_ to embed the RHPAM process engine in a Spring Boot application. The RHPAM process engine is used to coordinate the message flow between the services and advance the state of the Ride entity.
...
* The Dispatch service consumes a _RideRequestedEvent_ message from the _topic-ride-event_ topic.
* An instance of the Dispatch process is created. The process instance orchestrates the state of the Ride entity and the message exchange between the several services.
...
* _Signal_ event nodes are used to model the fact that the process instance is waiting for a certain type of message. When the service consumes a message, it finds the relevant process instance, and signals the process.
* Signal nodes are wait states, so at each signal the state of the process instance is peristed to the database.
...
Embedding the RHPAM process engine in a Spring Boot application is extremely easy when using the provided Spring Boot starters. Adding the `jbpm-spring-boot-starter-basic` to the project dependencies will configure all the services required to bootstrap the process engine, including persistence and transaction management. In the service implementation, the RHPAM Service APIs are used to interact with the process engine to for instance start and signal process instances.

== OpenShift Environment

An Openshift environment is provided to you to deploy and run the lab assets.
...
To login with the OpenShift `oc` client:
...
+
----
$ oc3 login https://master00-<GUID>.generic.opentlc.com -u user1
----
...
AMQ Streams uses the Operator model to deploy and manage Kafka (including Zookeeper) clusters. The AMQ Streams _Cluster Operator_ works in tandem with a Kafka _Custom Resource Definition_, which describes the target Kafka cluster. When a Kafka resource is deployed into an OpenShift namespace monitored by the Cluster Operator, the Operator deploys a corresponding Kafka cluster. The cluster consists of a Zookeeper ensemble, the Kafka cluster and the Entity Operator, which provides operator-style topic management via _KafkaTopic_ custom resources.
...
* The Kafka cluster is deployed in a project `kafka-cluster-user1`. In the OpenShift console, log in as `user1`, and navigate to the project where the Kafka cluster is deployed. Expect to see the following:
+
image::images/kafka_cluster_deployment.png[]
+
* *kafka-cluster-zookeeper* : Zookeeper ensemble consisting of 3 pods.
* *kafka-cluster-kafka*: Kafka broker cluster, consisting of 3 broker pods.
...
The Ansible playbook installs the Gogs server, including creating the admin user (`gogsadmin/admin123`) and the developer user (`developer/developer123`).
...
data:
  application-config.yaml: |-
    ...
    driver.assigned.min.delay: 3
    driver.assigned.max.delay: 6
...
When it comes to messaging, again some choices have to be made. Messaging brokers have been around for a long time, and considering the JMS standard in the Java EE world, JMS would be could be an obvious choice. However JMS only specifies an API, not the message format or wire protocol. With other words, JMS is not interoperable, even not between broker implementations. In a polyglot microservices world this is a huge drawback. AMQP on the other hand also defines the message format and wire protocol, making it interoperable between platforms and languages.

Recently, Apache Kafka is becoming more and more popular as a message exchange and data streaming platform. Originally developed at LinkedIn, it is a kind message broker but with a few twists to support pub/sub, extreme scalability and replaying of mesages. Other than with traditional brokers where message are removed from the broker once they are consumed by the known and registered consumers, in Kafka messages are retained (potentially) forever. In that regards it acts like a distributed journal, where consumers can start consuming from the beginning at any time. +
Kafka provides client for a large set of languages including Java, Golang, Javascript, .Net, making it very suitable to be used in polyglot microservices world.

For the use case of this labs, both a traditional message broker (which has support for topics) as well as Apache Kafka could be used. In fact, the initial version of this lab leveraged EnMasse, a version of the popular AMQ message broker.

AMQ Streams is a distribution of Apache Kafka, supported by Red Hat and optimized to run on OpenShift.
...
The choice to use two different runtimes was done on purpose to explore how messaging and in particular Apache Kafka, Jaeger integration and appliocation monitoring can be used on top of these runtimes. It is planned for further iterations of this lab to also leverage Thorntail (aka WildFly Swarm) and Fuse (Camel on Spring Boot).
...
The `Ride` entity is stored in the RHPAM database (to keep things simple - in a real life scenario you would probably store this data in its own data store). The `Ride` entity is added to the list of persistent classes in `META-INF/jbpm-persistence.xml`.
...
* Datasources:
** The RHPAM engine uses a JTA managed XA datasource. When using Quartz to handle persistent timers, a non-managed datasource is required for Quartz.
** The managed and the unmanaged datasources use a datasource connection pool powered by the Apache DBCP2 library.
** The datasources are configured as part of the RHPAM Spring Boot autoconfiguration.
...
** The Quartz scheduler is automatically created by the RHPAM engine if a application property `jbpm.quartz.enabled` is set to `true`. A Quartz configuration file needs to be provided - see the `jbpm-quartz.properties` file in the `etc` folder of the source code for the quartz properties file used. Additionally to use the database as job store application property `jbpm.quartz.db` needs to be set to true.


* RHPAM engine

* Kafka integration: Kafka integration is configured is the same way as in the Passenger service. Details can be found in the next paragraph.
** The Kafka Messagelisteners `DriverAssignedEventMessageListener`, `PassengerCanceledEventMessageListener` and `RideEventsMessageListener` interact with the RHPAM engine through the RHPAM services API. This interaction happens in a transactional context, managed by Spring's `TransactionTemplate`.
+
----
...
Kafka messaging on Spring Boot is made easy with the `spring-kafka` component. Kafka is configured in `PassengerServiceKafkaConfiguration` where various components are created:
...
The producer takes a `JsonObject` as payload. The structure of the JsonObject reflects the structure of the message.
...
The `java-kafka-client` and `opentracing-kafka-spring` libraries provide integration with Apache Kafka on the Spring platform. These libraries provide instrumented versions of `org.apache.kafka.clients.producer.Producer` and `org.apache.kafka.clients.consumer.Consumer` which add tracing spans to when consuming or producing Kafka messages.

* The tracing information is added as headers to Kafka messages
...
Vert.x provides some integration with OpenTracing, but only for the Vert.x Web component, not for the Vert.x Kafka client or the Vert.x event bus. +
...
* When the `MessageConsumerVerticle` receives a `AssignDriverCommand` message, the span information is extracted from the incoming Kafka message and a new span is created with the extracted span as parent span.
+
----
...
----
* In the `MessageProducerVerticle` the active span is extracted from the event bus message headers. A new span is created as a child span and added to the header section of the Kafka message.
+
----
